...
- **Mobile bottom-nav reserves space via `--bottom-nav-h`** on `:root` (0 by default, 64px at ≤720px). Subtracted from `.chat-page` and `.hs-settings-page`'s `100vh - var(--topbar-h) - var(--bottom-nav-h)` height calcs so the chat composer + settings rail don't slide under the tab bar. New full-viewport shells must subtract both.
