...
- **`components/FilterModal.jsx`** — Filter dialog opened from the timeline toolbar (and from the mobile topbar filter icon). Wraps the reusable `<Modal>`; manages a draft copy of `{event_type, person_ids, thread_ids}` and applies it on "Show results" so cancel is non-destructive. The disabled `search the timeline…` input also lives at the top of the dialog (it's the only access point to "search" on mobile, since the in-feed toolbar is hidden there).
- **`components/TimelineToolbar.jsx`** — Sticky toolbar on the timeline feed (desktop/tablet only — hidden at ≤720px). Search input (placeholder only, `⌘K` chip, disabled), Filter button with active count, and an "Add event" split button whose chevron opens a dropdown with the AI-caption-photo and EXIF-photo capture flows. On phone, `TimelineView` portals equivalent filter + add-with-dropdown icons into the topbar's `#topbar-actions` slot.
- **`components/YearRail.jsx`** — Year spine on the timeline (`.yearspine` / `.yr`). Each row is a pill with the year + a density bar whose width is `count / max(count) * 100%`. Active row inverts to dark + gold bar. Respects active filters via `GET /api/events/years`. Desktop: sticky vertical left column. ≤1080px: sticky horizontal strip under the topbar, placed *below* the feed header via `grid-template-areas`. A `useEffect` keeps the active pill in view by walking up to the nearest scrolling ancestor (`.yearspine` on desktop, `.yr-list` on mobile) and scroll-correcting only that container, never the page.
...
- **`components/Topbar.jsx`** — Hindsite wordmark + Timeline / Chat nav (text links, accent underline on the active page) + account chip on the far right (avatar initial + display name + email → dropdown with Settings + Sign out). On mount, a `ResizeObserver` publishes the topbar's rendered height as the `--topbar-h` CSS variable on `:root` so every sticky offset/scroll-margin/100vh-minus-topbar calc tracks the actual bar across breakpoints. Renders an empty `<div id="topbar-actions">` slot before the account chip; pages portal contextual icon buttons (e.g. TimelineView's mobile filter + add) into it. At ≤720px the text nav hides (replaced by `BottomNav`) and the account chip collapses to avatar-only.
...
- **`pages/TimelineView.jsx`** — `.tl-grid` uses `grid-template-areas` so the three pieces — `rail` (`<YearRail>`), `head` (italic accent-blue "Timeline." + mono `X events · since YYYY · N threads`), `feed` (`<TimelineToolbar>` + grouped events) — rearrange per breakpoint: desktop `"rail head" / "rail feed"` (rail spans both rows in a 140px left column), phone `"head" / "rail" / "feed"`. Per-day rows include the year above the date in the date column. On mount, the page binds to the topbar's `#topbar-actions` slot and (via `createPortal`) renders a filter icon (with an active-filter dot) plus a plus icon whose tap opens a dropdown matching the desktop split button (Add event / Photo with AI captions / Event from photo). The shared file inputs + handlers stay on the page. `[data-year-marker={year}]` lives on the first month-header of each year for the scrollspy + jump targets. Bidirectional infinite scroll: top sentinel calls `loadNewer` (with scroll-anchor preservation via `useLayoutEffect`), bottom sentinel calls `loadOlder`.
