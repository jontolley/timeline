...
- `AUTH_EMAIL_RATE_LIMIT` / `AUTH_EMAIL_RATE_WINDOW` — optional per-IP cap on the two Resend-sending endpoints (`POST /api/auth/request`, `POST /api/auth/request-code`); defaults to 5 requests per 3600s. Enforced in-process by `backend/ratelimit.py` (a `RateLimiter` instance in `routes/auth.py`), so the ceiling is per Fly machine — nginx can't do it because prod `/api` bypasses nginx via the Cloudflare Pages Function. Real client IP comes from `CF-Connecting-IP` (forwarded verbatim by the Pages Function), falling back to `X-Forwarded-For` then the socket peer. Over-limit returns 429 + `Retry-After`.
...
- **`lib/imagesToPdf.js`** — Client-side "combine images into a PDF" builder. `imagesToPdf(files, {name, onProgress, documentScan})` decodes each image (HEIC routed through a lazily-imported `heic2any`; everything else via the canvas `loadImage`/`renderResized` helpers exported from `uploads.js`), embeds one image per page (page sized to the resized image, no letterboxing) with a lazily-imported `pdf-lib`, and returns a single PDF `File` named with a GUID (R2 generates its own key, so the name is never user-visible). With `documentScan` set, each image is run through `lib/documentScan.js` first (crop + deskew) and the deskewed canvas feeds `renderResized` in place of the original. The result drops straight into the normal `uploadPdf` pipeline — no backend changes. Both heavy deps are dynamic `import()`s so they stay off the initial bundle.
- **`lib/documentScan.js` + `lib/documentScan.worker.js`** — Phase-1 document-scan spike for the combine-to-PDF flow. `scanDocument(img|canvas)` detects a page (Canny → contour → corner points → perspective warp, routine adapted from jscanify) and returns a deskewed/cropped canvas, or `null` when no plausible page is found (caller keeps the original). **OpenCV.js (~10MB WASM) runs in the Web Worker, not the main thread** — loading/instantiating it on the main thread freezes the whole UI, which is why the checkbox originally locked up the page. Workers have no DOM, so the client downscales the source to ≤2000px `ImageData`, **transfers** the buffer to the worker (zero-copy), and rebuilds a canvas from the returned RGBA pixels. `warmUpDocumentScan()` (called when the checkbox is ticked) posts a `warmup` message so the worker starts loading OpenCV before generate time. Auto-detect only — no manual corner adjustment; implausible detections (sliver `<15%` or whole-frame `>98%`) fall back to the original. **The jscanify npm package is deliberately NOT a dependency** — its Node build pulls a native `canvas`/`jsdom` that fails `node-gyp` in the Alpine Docker builder; its browser routines are inlined instead. OpenCV is bundled into the worker chunk and lazy-loaded, so the initial bundle is unchanged.
...
- **`components/LocationPicker.jsx`** — Leaflet + OSM with Nominatim geocoding (no API key, debounced 400ms). On mount, auto-geocodes name/address-without-coords and reverse-geocodes coords-without-name (so EXIF locations resolve to readable addresses). Restyled for the event form: search input has a leading map-pin icon (`.ef-ibox`/`.ef-lead`) + a trailing map-toggle icon button (replaces the old text toggle).
- **`components/DateField.jsx`** — Typed date input + custom calendar popover used by `EventForm`. `value`/`onChange` speak `'YYYY-MM-DD'` (the UTC-safe calendar-date contract); display is the long form, rendered/parsed without UTC shift. **The popover is portaled to `<body>` with fixed positioning** (computed from the input rect, flips up near the viewport bottom) so it escapes the range-reveal's `overflow:hidden` clip and layers above the map. A calendar pick sets the visible text directly (the focus-guarded value-sync effect would otherwise skip it).
- **`components/ThreadModal.jsx`** — Create/edit thread dialog (name + color swatches + visibility), extracted from `ThreadsSettings` so both Settings and `EventForm`'s "New thread" pill share it. Controlled via `modalState` + `setDraft`.
- **`components/Icons.jsx`** — Small lucide-style stroke icons (24×24, `currentColor`) for the event form + pickers. Inline SVG, no icon dependency.
...
- **`components/AddMediaButton.jsx`** — Shared "Add media" split button (mirrors the timeline's "Add event" control via the `.tl-add-*` classes). Main button opens the file picker; the chevron menu offers **Upload files** + **Combine images to PDF** (opens `CombinePdfModal`). Owns the hidden `<input>`, the menu, and the modal; the parent passes only `onFiles(files)` (picked files OR the single generated PDF) + a `busy` flag. Mounted on both `EventForm` (create) and `EventDetail`'s `MediaSection`.
- **`components/CombinePdfModal.jsx`** — The combine-to-PDF picker on the `Modal` shell: add images, reorder them into page order, generate. **Reordering works on touch via per-tile move ‹ › buttons** (native HTML5 drag-and-drop doesn't fire on touchscreens) with desktop drag-and-drop layered on top. HEIC tiles show a placeholder instead of an `<img>` preview (can't decode off Safari). A "these are photos of documents" checkbox enables `documentScan` (crop + deskew via `lib/documentScan.js`) and warms up the OpenCV worker on tick. Calls `imagesToPdf` and hands the `File` back via `onCreate`.
...
- **`pages/EventForm.jsx`** — Create + edit (same component, routed `/events/new` and `/events/:id/edit`). "Approach A" design adapted to Hindsite tokens (all classes `.ef-*`, scoped under `.eventform`). Thread = single-select pills + a "New thread" pill opening the shared `ThreadModal` (on create, refreshes the thread store and auto-selects the new thread). Date via `DateField`; "Add a time" / "Make it a range" dashed toggle pills replace the old checkboxes (range keeps an optional end **time**, unlike the mockup). Inline tag chips, people avatar pills, restyled `LocationPicker`, and a **fixed Save/Cancel footer** pinned at `bottom: var(--bottom-nav-h)` so it clears the mobile bottom-nav (the Save button is `type=submit form="eventForm"`, outside the `<form>`). **Create-page defaults:** date defaults to today (no time) unless a photo's EXIF supplies one; if a photo gives no GPS, a one-shot effect falls back to `navigator.geolocation` (reverse-geocoded via Nominatim) — best-effort, never overwrites an already-set location, create-only. Media section is create-only (edit manages media on the detail page). All existing logic preserved: photo-handoff prefill, AI-caption skeleton, media queue/posters, and the exact UTC ISO date/end-date submit.
...
- **Overlays must out-stack Leaflet maps.** Leaflet panes/controls use z-indexes up to ~1000 in the shared stacking context, so any fixed overlay that can appear over a map needs to sit above that: `.hs-modal-scrim` (1490), `.sheet-backdrop` confirm (1480), `.photo-busy-scrim` (1495), and the portaled `.ef-cal` (1200). `.location-map` also carries `isolation: isolate` to contain Leaflet's z-indexes. Anything new that overlays a map (the event form + event detail both render maps) must clear ~1000, or be portaled out — don't reintroduce a low z-index here.
