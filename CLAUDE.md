...
- **`components/EventCard.jsx`** — Timeline tile, rebuilt as a hover-lift card (`.event`). Mono meta row (category swatch + name + thread pill on the right + "shared" badge for cross-user events), sans 21px title, italic-serif location with an accent-colored map-pin SVG, description, photo grid that auto-picks 1/3/4 columns (1-col uses 16:9), then a `#tag` chip row when the event has tags, then people chips with colored dots. Whole card is clickable (`onClick={open}`); tagged with `data-event-id` (scroll anchor) and `data-event-date`.
