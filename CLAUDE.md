...
- **`components/FilterModal.jsx`** — Filter dialog opened from the timeline toolbar. Wraps the reusable `<Modal>`; manages a draft copy of `{event_type, person_ids, thread_ids}` and applies it on "Show results" so cancel is non-destructive.
- **`components/TimelineToolbar.jsx`** — Sticky toolbar on the timeline feed. Search input (placeholder only, `⌘K` chip, disabled), Filter button with active count, and an "Add event" split button whose chevron opens a dropdown with the AI-caption-photo and EXIF-photo capture flows.
- **`components/YearRail.jsx`** — Sticky vertical year spine on the timeline (`.yearspine` / `.yr`). Each row is a pill with the year + a density bar whose width is `count / max(count) * 100%`. Active row inverts to dark + gold bar. Respects active filters via `GET /api/events/years`. Collapses to a horizontal strip ≤1080px.
...
- **`components/Topbar.jsx`** — Hindsite wordmark + Timeline / Chat nav (rendered as text links with an accent-color underline on the active page, no pill backgrounds), and the account chip on the far right (avatar initial + display name + email → dropdown with Settings + Sign out). Esc and click-outside close the dropdown.
...
- **`pages/TimelineView.jsx`** — Two-column page: `.yearspine` (`<YearRail>`) on the left, `.tl-feed` on the right. Header is the giant italic accent-blue "Timeline." with a mono stats line (`X events · since YYYY · N threads`). `<TimelineToolbar>` sits sticky under the topbar inside the feed. Events group year → month → day; each day is a row with a date column and a dashed left spine, multiple same-day events stack inside. `[data-year-marker={year}]` lives on the first month-header of each year for the scrollspy + jump targets. Bidirectional infinite scroll: top sentinel calls `loadNewer` (with scroll-anchor preservation via `useLayoutEffect`), bottom sentinel calls `loadOlder`.
...
- **Sticky topbar offset lives in `--topbar-h: 80px`** on `:root` (sized to the actual rendered topbar — account chip is 30px avatar + padding, so the bar is ~79px tall). Consumed by `.tl-toolbar` and `.yearspine` sticky `top:`, `.tl-month` + `.event` `scroll-margin-top`, the fixed-height `.hs-settings-page` and `.chat-page` shells, and the runtime scrollspy threshold in `TimelineView`. Bump the var, everything follows. The previous 64px was a guess; don't ship a lower value without re-measuring.
