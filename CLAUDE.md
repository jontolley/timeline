...
- **`lib/imagesToPdf.js`** — Client-side "combine images into a PDF" builder. `imagesToPdf(files, {name, onProgress})` decodes each image (HEIC routed through a lazily-imported `heic2any`; everything else via the canvas `loadImage`/`renderResized` helpers exported from `uploads.js`), embeds one image per page (page sized to the resized image, no letterboxing) with a lazily-imported `pdf-lib`, and returns a single PDF `File` named with a GUID (R2 generates its own key, so the name is never user-visible). The result drops straight into the normal `uploadPdf` pipeline — no backend changes. Both heavy deps are dynamic `import()`s so they stay off the initial bundle.
