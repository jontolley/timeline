...
- **Sort by event date or date added.** The filter dialog has a "Sort by" toggle — order the timeline by each event's own date (default) or by when it was added. The feed, its month/year headers, and the year spine all follow the chosen field.
