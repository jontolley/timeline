...
router = APIRouter(prefix="/api/categories", dependencies=[Depends(require_auth)])
...
async def list_categories():
    cursor = categories_collection.find().sort("label", 1)
...
async def update_category(category_id: str, patch: CategoryUpdate):
...
async def delete_category(category_id: str):
    ...
    if not doc:
        raise HTTPException(status_code=404, detail="Category not found")
    # Block deletion if any event still uses this category.
    in_use = await events_collection.count_documents({"event_type": doc["name"]})
    if in_use:
        raise HTTPException(
            status_code=409,
